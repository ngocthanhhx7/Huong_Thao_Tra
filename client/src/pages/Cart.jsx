    ...
    return (
        <div className="luxury-page py-16 px-4">
            <div className="luxury-container max-w-5xl mx-auto">
                <div className="mb-10 text-center md:text-left animate-soft-rise">
                    <span className="luxury-kicker">🛒 Trà đã chọn</span>
                    <h1 className="font-display-h1 text-5xl md:text-6xl text-[#27451f] mt-4 mb-3">Giỏ hàng của bạn</h1>
                    <p className="text-gray-600 text-lg">Xem và quản lý các sản phẩm bạn đã chọn</p>
                </div>

                {!user ? (
                    <div className="luxury-card p-16 text-center border-dashed animate-soft-rise">
                        ...
                        <Link to="/login" className="luxury-btn luxury-btn-primary px-8 py-3.5 rounded-xl">
                            Đi đến đăng nhập
                        </Link>
                    </div>
                ) : cartItems.length === 0 ? (
                    <div className="luxury-card p-16 text-center border-dashed animate-soft-rise">
                        ...
                        <Link to="/teas" className="luxury-btn luxury-btn-primary px-8 py-3.5 rounded-xl">
                            Khám phá ngay
                        </Link>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 space-y-5">
                                {cartItems.map((item) => (
                                    <div key={item.id} className="luxury-card luxury-card-hover p-6 animate-soft-rise">
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
                                            {item.image ? <img src={item.image} alt={item.name} className="w-24 h-24 object-cover rounded-2xl bg-gray-100" /> : <div className="w-24 h-24 rounded-2xl bg-emerald-50 grid place-items-center text-3xl">🍵</div>}
                                            ...
                                            <div className="flex items-center gap-5 justify-between">
                                                <div className="flex items-center bg-gray-50 rounded-xl p-1 border border-gray-100">
                                                    <button onClick={() => item.quantity > 1 && updateQuantity(item.id, item.quantity - 1)} disabled={item.quantity <= 1} className="w-8 h-8 disabled:opacity-40">-</button>
                            ...
                            <div className="lg:col-span-1">
                                <div className="luxury-card p-7 lg:sticky lg:top-8 animate-soft-rise-delay">
                                    ...
                                    <Link to="/checkout" className="luxury-btn luxury-btn-primary w-full py-4 rounded-xl">
