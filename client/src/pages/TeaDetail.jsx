...
const TeaDetail = () => {
    ...
    if (loading) {
        return <div className="luxury-page"><div className="luxury-container text-center py-28 text-gray-500 font-medium">Đang tải chi tiết sản phẩm cao cấp...</div></div>;
    }

    if (!tea) {
        return <div className="luxury-page"><div className="luxury-container text-center py-28 text-gray-500 font-medium">Không tìm thấy sản phẩm.</div></div>;
    }

    return (
        <div className="luxury-page py-20 px-4">
            <div className="luxury-container max-w-7xl mx-auto space-y-10">
                {(message || error) && (
                    <div className={`luxury-card rounded-2xl px-5 py-4 border ${error ? 'bg-red-50/90 border-red-100 text-red-600' : 'bg-green-50/90 border-green-100 text-green-700'}`}>
                        {error || message}
                    </div>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_0.95fr] gap-10">
                    <div className="luxury-card overflow-hidden animate-soft-rise">
                        {tea.image ? <img src={tea.image} alt={tea.name} className="w-full h-[420px] object-cover hover:scale-105 transition-transform duration-700" /> : <div className="w-full h-[420px] grid place-items-center text-6xl bg-emerald-50">🍵</div>}
                    </div>

                    <div className="luxury-card p-8 space-y-6 animate-soft-rise-delay">
                        ...
                        <div>
                            <span className="luxury-kicker mb-4">🌿 Chi tiết sản phẩm</span>
                            <h1 className="font-display-h1 text-5xl md:text-6xl text-[#27451f]">{tea.name}</h1>
                        ...
                        <div>
                            <p className="text-sm font-semibold text-gray-700 mb-3">Thành phần nguyên liệu</p>
                            <div className="flex flex-wrap gap-3">
                                {(tea.ingredients || []).map((ingredient) => (
                                    <Link
                                        key={ingredient._id || ingredient.name}
                                        to={`/ingredients/${ingredient._id}`}
                                        className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-50/60 hover:bg-emerald-50 text-emerald-800 border border-emerald-100/40 text-sm font-extrabold hover:shadow-sm transition duration-300 animate-soft-rise"
                                    >
                                        {ingredient.image ? (
                                            <img
                                            ...
                                            <span className="w-6 h-6 rounded-full bg-emerald-100 text-emerald-800 flex items-center justify-center text-xs shrink-0 font-bold">
                                                🌿
                                            </span>
                                        )}
                        ...
                        <div className="flex flex-wrap gap-3">
                            <button onClick={addToCart} className="luxury-btn luxury-btn-primary px-6 py-3 rounded-2xl">
                                Thêm vào giỏ hàng
                            </button>
                            <Link to="/cart" className="luxury-btn luxury-btn-soft px-6 py-3 rounded-2xl">
                                Xem giỏ hàng
                            </Link>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-[1.15fr_0.85fr] gap-8">
                        ...
                        <div className="luxury-card p-8">
                            ...
                            <div className="space-y-6">
                                {reviews.length === 0 ? (
                                    ...
                                    reviews.map((review) => (
                                        <div key={review._id} className="rounded-3xl border border-emerald-100/70 bg-white/65 p-5">
                    ...
                    <div className="space-y-8">
                        {user && hasPurchased ? (
                            <form onSubmit={submitReview} className="luxury-card p-8 space-y-4">
                                ...
                                />
                                <button className="luxury-btn luxury-btn-primary w-full py-4 rounded-2xl">
                                    Gửi đánh giá
                                </button>
                            </form>
                        ) : (
                            <div className="luxury-card p-8 text-center space-y-3">
                                ...
                                {!user && (
                                    <Link to="/login" className="luxury-btn luxury-btn-primary px-6 py-2.5 rounded-xl text-sm min-h-0">
                                        Đăng nhập
                                    </Link>
                                )}
                            </div>
                        ...
                        <form onSubmit={submitProductFeedback} className="luxury-card p-8 space-y-4">
                            ...
                            <textarea
                                ...
                                className="w-full px-4 py-3 rounded-2xl border border-gray-200"
                            />
                            <button className="luxury-btn luxury-btn-soft w-full py-4 rounded-2xl">
