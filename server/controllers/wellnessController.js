...
const getDrinkSuggestion = async (req, res) => {
  console.log('[wellness] drink-suggest called');
  try {
...
Trả lời bằng tiếng Việt, định dạng JSON:
{
  "suggestions": [
    ...
    try {
      ...
      console.error('[wellness] drink-suggest Gemini error:', err.message, err.stack);
...
const coachCheckin = async (req, res) => {
  console.log('[wellness] coach-checkin called');
  try {
...
Dựa trên lịch sử sức khỏe gần đây, hãy đưa ra lời khuyên cá nhân hóa bằng tiếng Việt về việc nên uống trà gì, tập luyện thế nào. Ngắn gọn, ấm áp, dưới 150 từ.${contextStr}`;
    ...
    try {
      ...
      console.error('[wellness] coach-checkin Gemini error:', err.message, err.stack);
      aiMessage = 'Cảm ơn bạn đã check-in! Tôi sẽ phân tích và đưa ra lời khuyên phù hợp.';
    }
...
const coachChat = async (req, res) => {
  console.log('[wellness] coach-chat called');
  try {
...
Coach:`;
    ...
    try {
      ...
      console.error('[wellness] coach-chat Gemini error:', err.message, err.stack);
    }
...
const checkDrugInteraction = async (req, res) => {
  console.log('[wellness] drug-check called, body:', JSON.stringify(req.body).substring(0, 200));
  try {
...
Trả lời bằng tiếng Việt, định dạng JSON:
{
  "interactions": [
    ...
    try {
      console.log('[wellness] drug-check prompt:', prompt.substring(0, 200));
      ...
      console.error('[wellness] drug-check Gemini error:', err.message, err.stack);
      interactions = [];
    }
